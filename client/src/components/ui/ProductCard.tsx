...
export default function ProductCard({ product }: { product: ProductCardProps }) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    // Ripple animation
    gsap.fromTo(cardRef.current, { boxShadow: '0 0 0 4px rgba(107, 62, 38, 0.4)' }, {
  ...
  const handleWishlist = (e: React.MouseEvent) => {
    ...
    // Bounce animation
    gsap.fromTo(heartRef.current, { scale: 1 }, { scale: 1.4, duration: 0.15, ease: 'power2.out', yoyo: true, repeat: 1 });
  ...
  return (
    <div ref={cardRef} className="product-card group relative">
      {/* Image Container */}
      <div className="relative overflow-hidden rounded-t-2xl" style={{ aspectRatio: '1' }}>
        {/* Badges */}
        ...
        <div className="absolute inset-0 bg-primary-DEFAULT/60 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300">
          ...
          >
            <Eye size={18} className="text-primary-DEFAULT group-hover:text-white" />
          </Link>
          <button
      ...
      <div className="p-4">
        ...
        {/* Weight */}
        <p className="text-xs text-text-muted font-body mb-2">{product.weight}</p>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span className="text-xs text-text-muted font-body">({product.review_count})</span>
        </div>
