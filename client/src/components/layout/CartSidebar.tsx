...
import { X, ShoppingBag, Plus, Minus, Trash2, ArrowRight, Leaf, ShieldCheck } from 'lucide-react';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import {
  setCartOpen, updateQuantity, removeFromCart,
  selectCartItems, selectCartTotal, selectCartCount, selectCartIsOpen
...
export default function CartSidebar() {
  ...
  const handleUpdateQty = (id: number, currentQty: number, change: number) => {
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-xs font-body text-text-DEFAULT">
                  {remainingForFreeShipping > 0 ? (
                    <>Add <span className="font-bold text-primary-DEFAULT">₹{remainingForFreeShipping.toLocaleString()}</span> more for <span className="font-bold text-green-600">FREE delivery!</span></>
                  ) : (
                    <span className="font-bold text-green-600">🎉 Congratulations! You qualify for FREE delivery.</span>
                  )}
                </p>
              </div>
            ...
            {cartItems.length > 0 ? (
              cartItems.map((item) => (
                ...
                >
                  ...
                  <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-background border border-border-light flex-shrink-0">
                    <Image
                      src={item.thumbnail || '/images/categories/mixed-nuts.png'}
                      alt={item.name}
                      fill
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-xs text-text-muted font-body mt-0.5">{item.weight}</p>
                    </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center py-20">
                ...
                <p className="text-text-muted font-body text-sm max-w-xs mb-8">
                  Looks like you haven&apos;t added any premium dry fruits or nuts to your cart yet.
                </p>
                <button
                  onClick={handleClose}
            ...
            <div className="border-t border-border-DEFAULT p-6 bg-surface space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-base font-heading font-bold text-primary-DEFAULT">
                  ...
                  <span>
                    ₹{(cartTotal >= freeShippingThreshold ? cartTotal : cartTotal + 99).toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-1.5 text-xxs text-text-muted font-body pt-2">
                ...
                <span>Secure 256-bit SSL checkout. 100% satisfaction guarantee.</span>
              </div>
            </div>
          )}
        </div>
