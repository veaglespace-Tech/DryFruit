...
export default function Newsletter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    gsap.fromTo('.newsletter-success', { scale: 0.8, opacity: 0 }, {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(135deg, #F5EDE0 0%, #EFE7DD 100%)' }}
    >
      {/* Decorative Elements */}
      <div className="absolute top-0 right-0 w-72 h-72 rounded-full blur-3xl opacity-20" style={{ background: '#D4A95A' }} />
      <div className="absolute bottom-0 left-0 w-48 h-48 rounded-full blur-3xl opacity-15" style={{ background: '#6B3E26' }} />
      ...
      <div className="container-luxury relative">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="body-lead mt-4 mb-8">
            Subscribe to our newsletter for exclusive offers, new arrivals, healthy recipes, and tips on living your best nutritious life.
          </p>

          {/* Perks */}
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            {['Exclusive Discounts', 'New Arrivals First', 'Healthy Recipes', 'Seasonal Offers'].map((perk) => (
              <span key={perk} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-border-DEFAULT text-xs font-button font-semibold text-primary-DEFAULT">
          ...
          {!submitted ? (
            ...
            <div className="newsletter-success flex flex-col items-center gap-4 p-8 rounded-2xl bg-white border border-border-DEFAULT shadow-luxury">
              ...
              <div>
                <p className="font-heading text-lg font-semibold text-primary-DEFAULT mb-1">You&apos;re in! 🎉</p>
                <p className="text-text-muted font-body text-sm">Welcome to the NutriRoots family. Check your email for a special welcome offer!</p>
              </div>
            </div>
          )}
