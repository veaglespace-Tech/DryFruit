...
// Floating nut data
const floatingNuts = [
  { src: '/images/categories/almonds.png', size: 100, x: '8%', y: '20%', delay: 0, duration: 6 },
  { src: '/images/categories/cashews.png', size: 90, x: '90%', y: '15%', delay: 1.5, duration: 7 },
  { src: '/images/categories/walnuts.png', size: 110, x: '85%', y: '70%', delay: 0.5, duration: 5.5 },
  { src: '/images/categories/pistachios.png', size: 80, x: '5%', y: '75%', delay: 2, duration: 6.5 },
  { src: '/images/categories/dates.png', size: 95, x: '50%', y: '85%', delay: 1, duration: 7.5 },
];
  ...
  useGSAP(() => {
    ...
    nutsRef.current.forEach((nut, i) => {
      ...
      gsap.fromTo(nut,
        ...
        { opacity: 0.85, scale: 1, rotation: 0, duration: 1, delay: 0.8 + i * 0.15, ease: 'back.out(1.7)' }
      );
      // Continuous float
      gsap.to(nut, {
        y: -20 + Math.sin(i) * 10,
  ...
  return (
    ...
    >
      ...
      {floatingNuts.map((nut, i) => (
        <div
          key={i}
          ref={(el) => { nutsRef.current[i] = el; }}
          className="absolute z-10 pointer-events-none hidden lg:block"
          style={{ left: nut.x, top: nut.y }}
        >
          <div className="relative" style={{ width: nut.size, height: nut.size }}>
            <Image
              src={nut.src}
              alt="Floating nut"
              fill
      ...
      <div className="container-luxury relative z-20 py-20">
        <div className="max-w-3xl">
          ...
          <div className="hero-badge inline-flex items-center gap-2 px-4 py-2 rounded-full border border-white/30 bg-white/10 backdrop-blur-sm mb-6">
            ...
            <span className="text-white/90 text-sm font-button font-medium">100% Natural & Farm Fresh</span>
          ...
          <div ref={headlineRef} className="mb-6 overflow-hidden">
            ...
            <div className="hero-headline-line overflow-hidden">
              <h1 className="font-heading" style={{ fontSize: 'clamp(2.5rem, 7vw, 5.5rem)', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-0.02em', color: '#D4A95A' }}>
                Dry Fruits & Nuts
              </h1>
            </div>
            <div className="hero-headline-line overflow-hidden">
          ...
          <div ref={ctaRef} className="flex flex-wrap gap-4 mb-12">
            ...
            <Link href="/products" className="flex items-center gap-3 px-6 py-3 rounded-full border-2 border-white/40 text-white hover:bg-white/10 transition-all duration-300 font-button font-semibold">
        ...
        {/* Stats Row */}
        <div ref={statsRef} className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 max-w-2xl">
          {stats.map((stat) => (
            <div
              key={stat.label}
