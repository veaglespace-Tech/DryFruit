...
import { ShoppingBag, ArrowLeft, CheckCircle2, ShieldCheck, Mail, MapPin, Phone, User, Send } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      const itemsList = cartItems.map(item => `${item.name} (${item.weight}) x ${item.quantity}`).join(', ');
        ...
        <div className="container-luxury py-10">
          {!completed ? (
            <>
              {/* Back to cart */}
              <div className="mb-6">
                <Link href="/products" className="inline-flex items-center gap-1 text-xs font-button font-semibold text-text-muted hover:text-primary-DEFAULT transition-colors">
                  <ArrowLeft size={14} />
                  Back to Shop
                </Link>
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-8 items-start">
                    ...
                    <form onSubmit={handlePlaceOrder} className="space-y-4">
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="form-floating">
                          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder=" " required id="checkout-name" />
                          <label htmlFor="checkout-name">Full Name *</label>
                          <User size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted" />
                        </div>
                        <div className="form-floating">
                          <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder=" " required id="checkout-phone" />
                      ...
                      <div className="form-floating">
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder=" " required id="checkout-email" />
                        <label htmlFor="checkout-email">Email Address *</label>
                        <Mail size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted" />
                      </div>

                      <div className="form-floating">
                        <textarea value={address} onChange={e => setAddress(e.target.value)} placeholder=" " required rows={3} id="checkout-address" className="!py-3" />
                      ...
                      <div className="form-floating">
                        <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder=" " rows={2} id="checkout-notes" className="!py-3" />
                        <label htmlFor="checkout-notes">Notes / Special Instructions (Optional)</label>
                      </div>
                      ...
                      >
                        {submitting ? 'Processing Inquiry...' : `Submit Pre-Order Inquiry (₹${grandTotal.toLocaleString()})`}
                      </button>
                    </form>
                  </div>
                </div>
                ...
                <div className="lg:col-span-1 bg-white border border-border-DEFAULT rounded-3xl p-6 shadow-luxury space-y-6">
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-3 pr-2">
                    {cartItems.map((item) => (
                      <div key={item.id} className="flex gap-3 items-center">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-heading text-xs font-bold text-primary-DEFAULT truncate">{item.name}</h4>
                          <p className="text-xxs text-text-muted font-body mt-0.5">{item.weight} x {item.quantity}</p>
                        </div>
                    ...
                    {cartItems.length === 0 && (
                      <p className="text-xs text-text-muted font-body py-4 text-center">No items in cart</p>
                    )}
                  </div>
                  ...
                  <div className="border-t border-border-light pt-4 space-y-2">
                    <div className="flex justify-between text-xs font-body text-text-muted">
                      ...
                      <span className="font-semibold text-primary-DEFAULT">₹{cartTotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-xs font-body text-text-muted">
                  ...
                  <div className="flex items-start gap-2 p-3 rounded-xl bg-primary-50 border border-primary-DEFAULT/10 text-xxs text-text-muted font-body leading-normal">
                    ...
                    <span>Submission sends a Pre-Order Inquiry to our team. We will contact you immediately on WhatsApp/Phone to finalize payment and shipping.</span>
                  </div>
                </div>
              </div>
            </>
            ...
            <div className="max-w-xl mx-auto text-center py-16 bg-white border border-border-DEFAULT rounded-3xl p-8 md:p-12 shadow-luxury space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-text-muted font-body text-sm px-4">
                  Thank you, <strong className="text-primary-DEFAULT">{name}</strong>. Your pre-order request has been logged. Our sales support desk will contact you shortly to finalize details.
                </p>
              </div>
              ...
              <div className="pt-6 border-t border-border-light flex flex-wrap justify-center gap-4">
                ...
                <Link href="/user/dashboard" className="px-6 py-3 border border-border-DEFAULT text-text-muted hover:bg-background rounded-xl font-button text-xs transition-all">
                  Check Dashboard
                </Link>
              </div>
            </div>
