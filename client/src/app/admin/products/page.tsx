...
export default function AdminProductsPage() {
  ...
  const filtered = products.filter(p => {
    ...
    const matchSearch = p.name.toLowerCase().includes(search.toLowerCase()) || categoryName.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="bg-white border border-border-DEFAULT rounded-2xl overflow-hidden shadow-card">
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-xs text-text-muted font-body">Loading products...</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map((product) => (
                  <tr key={product.id} className="border-b border-border-light last:border-0 hover:bg-background transition-colors">
                    ...
                    <td className="p-4 text-text-muted font-body text-sm">{product.category?.name || 'Unassigned'}</td>
