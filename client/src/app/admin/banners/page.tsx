...
import { Image as ImageIcon, Plus, Trash2, Calendar, Eye } from 'lucide-react';
      ...
      <div className="lg:col-span-2 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card space-y-4">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-text-muted font-body">Loading banners...</p>
          </div>
        ) : banners.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-border-DEFAULT rounded-xl">
            ...
            <p className="font-heading text-primary-DEFAULT font-semibold text-sm">No Banners Found</p>
            <p className="text-xxs text-text-muted font-body mt-1">Upload a hero banner to manage display sliders.</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            {banners.map(banner => (
              <div key={banner.id} className="border border-border-light rounded-xl overflow-hidden shadow-sm flex flex-col bg-white">
                ...
                <div className="p-3 flex-1 flex flex-col justify-between gap-3">
                  <div>
                    <h3 className="font-heading text-xs font-bold text-primary-DEFAULT truncate">{banner.title}</h3>
                    {banner.subtitle && <p className="text-xxs text-text-muted truncate">{banner.subtitle}</p>}
                  </div>
                  <div className="flex justify-between items-center border-t border-border-light pt-2">
                    <span className="text-xxs text-green-700 bg-green-50 px-2 py-0.5 rounded-full font-bold">Active</span>
                    <button
      ...
      <div className="lg:col-span-1 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card h-fit space-y-4">
        ...
        <form onSubmit={handleAddBanner} className="space-y-4">
          <div className="form-floating">
            <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder=" " required id="banner-title" />
            <label htmlFor="banner-title">Banner Title *</label>
          </div>

          <div className="form-floating">
            <input type="text" value={subtitle} onChange={e => setSubtitle(e.target.value)} placeholder=" " id="banner-sub" />
            <label htmlFor="banner-sub">Subtitle / Slogan</label>
          </div>

          <div className="form-floating">
            <input type="text" value={linkUrl} onChange={e => setLinkUrl(e.target.value)} placeholder=" " id="banner-link" />
            <label htmlFor="banner-link">Target Link URL</label>
          </div>

          <div className="form-floating">
            <select value={position} onChange={e => setPosition(e.target.value)} id="banner-pos" className="w-full border border-border-DEFAULT rounded-xl px-3 py-3 text-xs bg-white text-text-DEFAULT outline-none focus:border-primary-DEFAULT transition-all">
          ...
          <div className="space-y-1">
            <label className="text-xxs font-bold text-primary-DEFAULT block">Select Banner Image *</label>
            <input
              type="file"
              accept="image/*"
