      ...
      <div className="lg:col-span-2 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card space-y-4">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-text-muted font-body">Loading testimonials...</p>
          </div>
          ...
          <div className="text-center py-12 border border-dashed border-border-DEFAULT rounded-xl">
            ...
            <p className="font-heading text-primary-DEFAULT font-semibold text-sm">No Testimonials Found</p>
            <p className="text-xxs text-text-muted font-body mt-1">Add client reviews to display in the website slider.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {testimonials.map(testimonial => (
              <div key={testimonial.id} className="p-4 border border-border-light rounded-xl bg-white shadow-sm flex items-start gap-4">
                ...
                <div className="flex-1 space-y-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      <h3 className="font-heading text-xs font-bold text-primary-DEFAULT truncate">{testimonial.name}</h3>
                      <p className="text-[10px] text-text-muted">{testimonial.role || 'Happy Customer'}</p>
                    </div>
                    <button
                  ...
                  <div className="flex gap-0.5">
                    {Array.from({ length: testimonial.rating }).map((_, i) => (
                      <Star key={i} size={10} className="fill-accent-DEFAULT text-accent-DEFAULT" />
                    ))}
                  </div>
                  <p className="text-xxs text-text-DEFAULT leading-relaxed italic">"{testimonial.comment}"</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="lg:col-span-1 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card h-fit space-y-4">
        ...
        <form onSubmit={handleAddTestimonial} className="space-y-4">
          <div className="form-floating">
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder=" " required id="test-name" />
            <label htmlFor="test-name">Client Name *</label>
          </div>

          <div className="form-floating">
            <input type="text" value={role} onChange={e => setRole(e.target.value)} placeholder=" " id="test-role" />
            <label htmlFor="test-role">Client Role / Profession</label>
          </div>

          <div className="form-floating">
            <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder=" " required rows={3} id="test-comment" className="!py-3" />
            <label htmlFor="test-comment">Comment / Feedback *</label>
          </div>

          <div className="form-floating">
            <select value={rating} onChange={e => setRating(parseInt(e.target.value))} id="test-rating" className="w-full border border-border-DEFAULT rounded-xl px-3 py-3 text-xs bg-white text-text-DEFAULT outline-none focus:border-primary-DEFAULT transition-all">
          ...
          <div className="space-y-1">
            <label className="text-xxs font-bold text-primary-DEFAULT block">Select Client Avatar (Optional)</label>
            <input
              type="file"
              accept="image/*"
