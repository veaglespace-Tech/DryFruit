      ...
      <div className="lg:col-span-2 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card space-y-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 border border-dashed border-border-DEFAULT rounded-xl">
            ...
            <p className="font-heading text-primary-DEFAULT font-semibold text-sm">No FAQs Found</p>
            <p className="text-xxs text-text-muted font-body mt-1">Create frequently asked questions for client self-help.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {faqs.map(faq => (
              <div key={faq.id} className="p-4 border border-border-light rounded-xl bg-white shadow-sm flex flex-col gap-2">
                ...
                <div>
                  <h3 className="font-heading text-xs font-bold text-primary-DEFAULT">Q: {faq.question}</h3>
                  <p className="text-xxs text-text-DEFAULT leading-relaxed mt-1">A: {faq.answer}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="lg:col-span-1 bg-white border border-border-DEFAULT rounded-2xl p-5 shadow-card h-fit space-y-4">
        ...
        <form onSubmit={handleAddFAQ} className="space-y-4">
          <div className="form-floating">
            <input type="text" value={question} onChange={e => setQuestion(e.target.value)} placeholder=" " required id="faq-q" />
            <label htmlFor="faq-q">Question *</label>
          </div>

          <div className="form-floating">
            <textarea value={answer} onChange={e => setAnswer(e.target.value)} placeholder=" " required rows={4} id="faq-a" className="!py-3" />
            <label htmlFor="faq-a">Answer *</label>
          </div>

          <div className="form-floating">
            <select value={category} onChange={e => setCategory(e.target.value)} id="faq-cat" className="w-full border border-border-DEFAULT rounded-xl px-3 py-3 text-xs bg-white text-text-DEFAULT outline-none focus:border-primary-DEFAULT transition-all">
